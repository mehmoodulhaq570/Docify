...
class ConverterGUI(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        ...
        # Option: Prefer Microsoft Word (Windows) for PDF→DOCX (higher fidelity)
        word_row = QtWidgets.QHBoxLayout()
        self.word_checkbox = QtWidgets.QCheckBox(
            "Prefer Microsoft Word (Windows) — higher fidelity"
        )
        self.word_checkbox.setToolTip(
            "If checked, Docify will try to use Microsoft Word via COM (requires Word + pywin32) before other methods."
        )
        word_row.addWidget(self.word_checkbox)

        # Button to install pywin32
        btn_install_pywin32 = QtWidgets.QPushButton("Install pywin32")
        btn_install_pywin32.setStyleSheet(self.button_style(accent=True))
        btn_install_pywin32.setFixedWidth(120)
        btn_install_pywin32.setToolTip(
            "Click to install pywin32 package (required for Word COM)"
        )
        btn_install_pywin32.clicked.connect(self.install_pywin32)
        word_row.addWidget(btn_install_pywin32)
        word_row.addStretch()
        ...
        self.percent_label.setStyleSheet(
            "color: #353b48; font-size: 14px; margin-left: 8px;"
        )
        ...
        btn_word2pdf.clicked.connect(
            lambda: self.run_conversion(converters.word_to_pdf, ".pdf")
        )
        ...
        btn_pdf2word.clicked.connect(
            lambda: self.run_conversion(
                lambda a, b: converters.pdf_to_word(
                    a,
                    b,
                    preserve_images=True,
                    preserve_tables=True,
                    prefer_word=self.word_checkbox.isChecked(),
                ),
                ".docx",
            )
        )
        ...
        btn_xlsx2csv.clicked.connect(
            lambda: self.run_conversion(converters.xlsx_to_csv, ".csv")
        )
        ...
        btn_csv2xlsx.clicked.connect(
            lambda: self.run_conversion(converters.csv_to_xlsx, ".xlsx")
        )
    ...
    def install_pywin32(self) -> None:
        """Install pywin32 package via pip in a background thread."""
        reply = QtWidgets.QMessageBox.question(
            self,
            "Install pywin32",
            "This will run 'pip install pywin32'. Continue?",
        ...
        self.status.setText("Installing pywin32...")
                ...
                try:
                    ...
                    if result.returncode == 0:
                        self.finished_signal.emit(
                            True, "pywin32 installed successfully!"
                        )
                    else:
                        self.finished_signal.emit(
                            False, f"pip install failed: {result.stderr[:200]}"
                        )
        ...
        def on_finished(success: bool, message: str) -> None:
            if success:
                ...
                self.status.setStyleSheet(
                    "color: #44bd32; font-size: 15px; margin: 16px;"
                )
                QtWidgets.QMessageBox.information(self, "Success", message)
            else:
                self.status.setText(f"Error: {message}")
                self.status.setStyleSheet(
                    "color: #e84118; font-size: 15px; margin: 16px;"
                )
                QtWidgets.QMessageBox.critical(self, "Installation Error", message)
    ...
    def run_conversion(
        self, func: Callable[[str, str], None], output_ext: str | None = None
    ) -> None:
        ...
        if not inp or not out:
            ...
            QtWidgets.QMessageBox.warning(
                self, "Missing File", "Please select both input and output files."
            )
            return
        ...
        class ConversionWorker(QtCore.QThread):
            ...
            def run(self) -> None:
                try:
                    ...
                    Exception
                ) as exc:  # noqa: BLE001 - keep broad catch to relay back to UI
                    self.finished_signal.emit(False, str(exc))
        ...
        def on_finished(success: bool, message: str) -> None:
            ...
            if success:
                ...
                self.status.setStyleSheet(
                    "color: #44bd32; font-size: 17px; font-weight: 600; margin: 16px;"
                )
                QtWidgets.QMessageBox.information(
                    self, "Conversion Complete", f"File saved as: {message}"
                )
            else:
                self.status.setText(f"Error: {message}")
                self.status.setStyleSheet(
